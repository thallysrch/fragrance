...
export async function runSeed(prisma: PrismaClient) {
  ...
  for (const p of data) {
    ...
    if (found) { created[p.name] = found.id; continue }
    const c = await prisma.perfume.create({ data: p as any })
    created[p.name] = c.id
  }
  ...
  for (const s of simPairs) {
    ...
    if (!exists) await prisma.similarity.create({ data: s })
  }
  ...
  if (!r1) await prisma.review.create({ data: { userId: u.id, perfumeId: azulId, rating: 5, comment: 'Lembra bastante o Bleu!' } })
  const r2 = await prisma.review.findFirst({ where: { userId: u.id, perfumeId: citricoId } })
  if (!r2) await prisma.review.create({ data: { userId: u.id, perfumeId: citricoId, rating: 4, comment: 'Excelente custo-benefício.' } })
}
